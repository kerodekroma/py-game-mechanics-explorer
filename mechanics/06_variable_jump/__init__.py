...
pygame.display.set_caption("mechanics > variable jumping")
...
# extra configs to check the key up event
jump_key_held = False
jump_hold_time = 0
MAX_HOLD_TIME = 0.15 # 150ms for extra jump boost
...
while True:
    ...
    if jump_key_held and jump_hold_time < MAX_HOLD_TIME:
        jump_hold_time += dt
        dude_vel_y -= JUMP_SPEED * dt # Extra upward force
    ...
    # Jump time
    text_content = f"Jump hold time: {jump_hold_time:.2f}"
    text_surface = pixel_font.render(text_content, True, palette[2])
