...
# python -m mechanics.25_homing_missile_flocking
...
class Missile:
    ...
    def __init__(self, x=100, y=100, wobble=WOBBLE_LIMIT):
        ...
        self.img = pygame.image.load(
            './assets/img/missile28x20.png').convert_alpha()
    ...
    def avoid(self, siblings):
        ...
        for m in siblings:
            ...
            # Calculate distance
            distance = self.get_distance((self.x, self.y), (m.x, m.y))
            ...
            if distance < self.AVOID_DISTANCE:
                ...
                if random.random() > 0.5:  # 50% chance to zig or zag
                    avoid_angle *= -1  # Reverse direction
                # Steer away by adjusting the angle
                self.angle += math.degrees(avoid_angle)
            ...
            # if distance < self.AVOID_DISTANCE and distance > 0:
            #     # Calculate a repulsion force proportional to the overlap
            #     force = (self.AVOID_DISTANCE - distance) / self.AVOID_DISTANCE
            #     angle = math.atan2(dy, dx)
            #     # Apply repulsion force in the opposite direction
            #     # Adjust the factor as needed
            #     self.vel_x -= math.cos(angle) * force * 50 * dt
            #     # Adjust the factor as needed
            #     self.vel_y -= math.sin(angle) * force * 50 * dt
            # END USING FORCE
    ...
    def update(self, mouse_pos, siblings):
        # avoid logic
        self.avoid(siblings)
        ...
        # Update rotation
        self.missile_img_rotated = pygame.transform.rotate(
            self.img, -self.angle)
        self.missile_rect_rotated = self.missile_img_rotated.get_rect(
    ...
    def render(self, screen):
        ...
        screen.blit(self.missile_img_rotated,
                    self.missile_rect_rotated.topleft)
...
while True:
    ...
    for missile in missiles:
        missile.update(mouse_pos, missiles)
        missile.render(screen)

    # DEBUG COLLISIONS
    for missile in missiles:
        pygame.draw.circle(screen, palette[3], (int(
            missile.x), int(missile.y)), Missile.AVOID_DISTANCE, 1)
